...
export const mockGistModel = (): LoadUserGistList.Model => ({
...
export const mockGistListModel = (): LoadUserGistList.Model[] => ([
  mockGistModel(),
  mockGistModel(),
  mockGistModel()
])

export class ILoadUserGistListSpy implements ILoadUserGistList {
  callsCount = 0
  gists = mockGistListModel()

  async loadAll(): Promise<LoadUserGistList.Model[]> {
    this.callsCount++
    return this.gists
  }
}