...
export const mockUserModel = (): LoadUserFollowersList.Model => ({
...
export const mockUserListModel = (): LoadUserFollowersList.Model[] => ([
  mockUserModel(),
  mockUserModel(),
  mockUserModel()
])

export class ILoadUserListSpy implements ILoadUserFollowersList {
  callsCount = 0
  users = mockUserListModel()

  async loadAll(): Promise<LoadUserFollowersList.Model[]> {
    this.callsCount++
    return this.users
  }
}